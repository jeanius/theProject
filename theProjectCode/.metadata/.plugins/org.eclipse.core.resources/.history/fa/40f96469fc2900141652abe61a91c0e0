	...
	public void saveLoad(){
		...
		dialog.addListener(new FileChooserDialog.OnFileSelectedListener() {
			...
			public void onFileSelected(Dialog source, File file) { //Load File
				...
				game.showActivityMessage("File selected: " + file.getName());
			}
			...
			public void onFileSelected(Dialog source, File folder, String name) { //Save File
				...
				if (null != fw){
					game.showActivityMessage("File created: " + folder.getName() + "/" + name);
				}
				else { //If user is denied access to folder
					game.showActivityMessage("You need to save to a storage folder, like 'Documents'");
				}
			}
		});
		...
		for (Instruction i: instructionList){
			...
			if (i instanceof Box){
				...
				if (box.getType()){ //Type of instruction
					sb.append(context.getString(R.string.inc));
				}

				else {
					sb.append(context.getString(R.string.deb));
				}
				...
				sb.append(context.getString(R.string.endText));
